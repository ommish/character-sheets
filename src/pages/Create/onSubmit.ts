...
export const onSubmit: FormEventHandler = (e) => {
  e.preventDefault();
  ...
  const saves = ABILITIES.reduce(
    (acc, ability) => ({
      ...
      [ability]: {
        ...
        notes: (
          document.getElementById(`saves.${ability}.notes`) as HTMLInputElement
        ).value,
      },
    }),
    {},
  ...
  PROFICIENCIES.forEach((proficiency) => {
    proficiencies[proficiency] = new Array(
      document.getElementById(`proficiencies.${proficiency}`)!.children.length -
        1,
    )
      .fill(null)
      .map((_, i) => ({
  ...
  const equipment = new Array(
    ...
    .fill(null)
    .map((_, i) => ({
      name: (document.getElementById(`equipment.${i}.name`) as HTMLInputElement)
        .value,
      notes: (
  ...
  const features = new Array(
    ...
    .fill(null)
    .map((_, i) => ({
      name: (document.getElementById(`features.${i}.name`) as HTMLInputElement)
        .value,
      description: (
        document.getElementById(`features.${i}.description`) as HTMLInputElement
      ).value,
      remainingUses: null,
    }));
  ...
  const weapons = new Array(
    ...
    .map((_, i) => ({
      ...
      damage: {
        die: (
          document.getElementById(`weapons.${i}.damage.die`) as HTMLInputElement
        ).value as Die,
        count:
          parseInt(
            (
      ...
      remainingUses: null,
      notes: (document.getElementById(`weapons.${i}.notes`) as HTMLInputElement)
        .value,
    }));
  ...
  window.location.assign(`/${character.name}`);
};
